...
* On Linux, Nix can now run builds in a user namespace where they run
  ...
  [system feature]: @docroot@/command-ref/conf-file.md#conf-system-features

* Nix can now automatically pick UIDs for builds, removing the need to
  create `nixbld*` user accounts. See [`auto-allocate-uids`].

  [`auto-allocate-uids`]: @docroot@/command-ref/conf-file.md#conf-auto-allocate-uids

* On Linux, Nix has experimental support for running builds inside a
  cgroup. See
  [`use-cgroups`](@docroot@/command-ref/conf-file.md#conf-use-cgroups).
